...
Region.prototype.update = function () {
    ...
    var layout = this.layouts[this.views.length];
    ...
    //Rebalance layout
    //Assuming wider than tall
    var nView = this.views.length;

    var rows = layout[1];
    var cols = Math.ceil(nView/rows);
    ...
    xOffset += (this.dimensions.width - (viewSize[0]*cols))/2;
    yOffset += (this.dimensions.height - (viewSize[1]*rows))/2;
    ...
    this.views.forEach(function (view, i) {
        ...
        x = xOffset + viewSize[0] * col;
        y = yOffset + viewSize[1] * row;
